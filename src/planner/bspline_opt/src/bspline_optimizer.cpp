...
namespace ego_planner
{
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    constexpr int ENOUGH_INTERVAL = 2; // 保证空闲区域足够大才能分隔出两段障碍区域
    ...
    for (int i = order_; i <= i_end; ++i)
    {
      ...
      for (double a = 1.0; a >= 0.0; a -= step_size)
      {
        ...
        if (occ && !last_occ)
        {
          // 为了防止两段障碍物之间有一段很小的空闲区域，被认为是一段新障碍物的开始
          // ----------  ---------------------
          //           ^
          //           |
          //           这里只有很小的一段空闲，其实不应该认为是新的一段障碍物区域，而是将它们融为一体，这就能理解 same_occ_state_times的作用了，从字面意义上看是说保持相同状态的次数，对于保持较短的，前后合并
        ...
        // 这里与上面的注释是一个意思，之所以有一个 flag_got_end_maybe 就是因为为了避免一小段空闲这种情况
    ...
    /*** calculate bounds ***/
    //        空闲段             障碍物段             空闲段         障碍物段         空闲段
    //    |            |xxxxxxxxxxxxxxxxxxxxx|            |xxxxxxxxxxxxxxxxx|              |
    //          ^                                   ^                               ^
    //          |                                   |                               |
    //     第一段id_low_bound                 第一段id_up_bound
    // 其实就是将每个障碍物段计算出它们一定裕度情况下的上下界，这个上下界使用控制点 id 表示的
    ...
    // 如果一段包含的控制点太少就不将它向前后都延伸一下，但不能延伸超过上面定义的 bound 范围
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        // 论文中说的 p 是在障碍物表面的一个点，因为代码中不可能获取障碍物表面，而是先在引导路径上找一点，然后往回退（以 resolution 为步长），退到靠近障碍物表面
        if (got_intersection_id >= 0)
        {
          ...
          if (length > 1e-5)
          {
            for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
            {
              ...
              // 退到障碍物里了，或者这是在两段障碍物的小间隙里（都已经退到起点了）
              if (occ || a < grid_map_->getResolution())
              {
                // 我们需要的是障碍物表面而不是障碍物里面，所以如果是到障碍物里面了就在向前进一步
                if (occ)
